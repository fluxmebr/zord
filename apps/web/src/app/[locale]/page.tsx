    ...
    <div className="min-h-screen bg-zord-bg text-zord-text">
      ...
      <section className="border-b border-zord-border bg-gradient-to-b from-zord-bg via-zord-muted/5 to-zord-bg py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          {/* ── PAINEL PRINCIPAL ── */}
          {demoTab === 'dashboard' && (
            <DemoShell title={p.dashboard.title} badge={p.liveBadge} badgeColor="text-green-400" pulse={pulse}>
              {/* Stats row */}
              <div className="grid grid-cols-2 gap-3 border-b border-zord-border p-4 sm:grid-cols-4">
                {[
                  { v: '12', l: p.dashboard.stats[0], c: 'text-zord-accent', bar: 'bg-zord-accent' },
                  { v: '847', l: p.dashboard.stats[1], c: 'text-purple-400', bar: 'bg-purple-400' },
                  { v: '2.341', l: p.dashboard.stats[2], c: 'text-yellow-400', bar: 'bg-yellow-400' },
                  { v: '8', l: p.dashboard.stats[3], c: 'text-red-400', bar: 'bg-red-400' },
                ].map((s) => (
              ...
              <div className="grid grid-cols-1 gap-0 sm:grid-cols-3">
                ...
                <div className="col-span-2 border-r border-zord-border p-4">
                  <div className="mb-3 text-[9px] font-bold uppercase tracking-widest text-zord-text-muted">{p.dashboard.activityLabel}</div>
                  {[
                    { time: '14:38', color: 'text-yellow-400', icon: '⊟' },
                    { time: '14:35', color: 'text-purple-400', icon: '◈' },
                    { time: '14:31', color: 'text-zord-accent', icon: '⬢' },
                    { time: '14:22', color: 'text-green-400', icon: '⊜' },
                    { time: '14:15', color: 'text-orange-400', icon: '◉' },
                    { time: '13:58', color: 'text-red-400', icon: '⊗' },
                  ].map((ev, i) => (
                    <div key={i} className="flex gap-3 border-b border-zord-border/30 py-2 last:border-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[9px] font-bold ${ev.color}`}>{p.dashboard.users[i]}</span>
                        </div>
                        <p className="text-[10px] leading-snug text-zord-text-muted">{p.dashboard.activity[i]}</p>
                      </div>
                    </div>
                  ))}
                </div>
                {/* Alert panel */}
                <div className="p-4">
                  <div className="mb-3 text-[9px] font-bold uppercase tracking-widest text-zord-text-muted">{p.dashboard.alertsLabel}</div>
                  {[
                    { sev: p.severity.critical, time: '14:31', c: 'border-red-700/60 bg-red-950/20 text-red-400' },
                    { sev: p.severity.critical, time: '13:44', c: 'border-red-700/60 bg-red-950/20 text-red-400' },
                    { sev: p.severity.high, time: '12:15', c: 'border-orange-700/60 bg-orange-950/20 text-orange-400' },
                    { sev: p.severity.high, time: '11:50', c: 'border-orange-700/60 bg-orange-950/20 text-orange-400' },
                    { sev: p.severity.medium, time: '10:22', c: 'border-yellow-700/60 bg-yellow-950/20 text-yellow-400' },
                    { sev: p.severity.info, time: '09:00', c: 'border-zord-border bg-zord-surface/30 text-zord-text-muted' },
                  ].map((a, i) => (
                    <div key={i} className={`mb-2 rounded border p-2 ${a.c}`}>
                      ...
                      <p className="mt-0.5 text-[9px] leading-snug">{p.dashboard.alerts[i]}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* ── COFRE DE EVIDÊNCIAS ── */}
          {demoTab === 'vault' && (
            <DemoShell title={p.vault.title} badge={p.vault.badge} badgeColor="text-green-400" pulse={pulse}>
              <div className="grid grid-cols-1 sm:grid-cols-5">
                ...
                <div className="col-span-3 overflow-x-auto border-r border-zord-border">
                  <table className="w-full min-w-[500px] text-[10px]">
                    ...
                    <tbody>
                      {[
                        { id: 'EVD-001', name: 'contrato_black_sea_2024.pdf', type: p.vault.types.pdf, size: '2.4 MB', cls: 'secret', chain: true, sel: true, color: 'text-yellow-400' },
                        { id: 'EVD-002', name: 'audio_interceptado_24abr.mp3', type: p.vault.types.audio, size: '18.7 MB', cls: 'confidential', chain: true, sel: false, color: 'text-blue-400' },
                        { id: 'EVD-003', name: 'foto_vigilancia_dubai_03.jpg', type: p.vault.types.image, size: '4.1 MB', cls: 'confidential', chain: true, sel: false, color: 'text-blue-400' },
                        { id: 'EVD-004', name: 'wire_transfer_85k_mirka.xlsx', type: p.vault.types.sheet, size: '891 KB', cls: 'secret', chain: true, sel: false, color: 'text-yellow-400' },
                        { id: 'EVD-005', name: 'passaporte_voronov_scan.pdf', type: p.vault.types.pdf, size: '1.2 MB', cls: 'secret', chain: false, sel: false, color: 'text-red-400' },
                        { id: 'EVD-006', name: 'historico_celular_abr2024.csv', type: p.vault.types.data, size: '3.8 MB', cls: 'secret', chain: true, sel: false, color: 'text-yellow-400' },
                        { id: 'EVD-007', name: 'imagem_satelite_armazem.tiff', type: p.vault.types.image, size: '22.1 MB', cls: 'ultraSecret', chain: true, sel: false, color: 'text-red-400' },
                      ].map((f) => (
                        <tr key={f.id} className={`border-b border-zord-border/30 transition-colors hover:bg-zord-surface/30 ${f.sel ? 'bg-zord-accent/5' : ''}`}>
                          ...
                          <td className="px-3 py-2">
                            <span className={`rounded border px-1.5 py-0.5 text-[8px] font-bold ${f.cls === 'ultraSecret' ? 'border-red-700/60 bg-red-950/20 text-red-400' : f.cls === 'secret' ? 'border-yellow-700/60 bg-yellow-950/20 text-yellow-400' : 'border-blue-700/60 bg-blue-950/20 text-blue-400'}`}>
                              {p.classification[f.cls as 'secret' | 'confidential' | 'ultraSecret']}
                            </span>
                          </td>
                          <td className="px-3 py-2">
                            {f.chain
                              ? <span className="font-mono text-[9px] text-green-400">{p.vault.integrityOk}</span>
                              : <span className="font-mono text-[9px] text-red-400">{p.vault.integrityPending}</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="col-span-2 p-4">
                  <div className="mb-3 text-[9px] font-bold uppercase tracking-widest text-zord-text-muted">{p.vault.detailLabel}</div>
                  <div className="space-y-2">
                    {[
                      { k: p.vault.detailKeys[0], v: 'contrato_black_sea_2024.pdf' },
                      { k: p.vault.detailKeys[1], v: 'a8f3c91d2e...4b7f' },
                      { k: p.vault.detailKeys[2], v: '14 ABR 2024 09:22 UTC' },
                      { k: p.vault.detailKeys[3], v: p.vault.collectedByValue },
                      { k: p.vault.detailKeys[4], v: 'OP-NIGHTHAWK-2024' },
                    ].map(({ k, v }) => (
                  ...
                  <div className="mt-4">
                    <div className="mb-2 text-[9px] font-bold uppercase tracking-widest text-zord-text-muted">{p.vault.custodyLabel}</div>
                    {[
                      { step: 1, action: p.vault.custodyActions[0], by: p.vault.custodyBy[0], date: '14 ABR 09:22', ok: true },
                      { step: 2, action: p.vault.custodyActions[1], by: p.vault.custodyBy[1], date: '14 ABR 09:23', ok: true },
                      { step: 3, action: p.vault.custodyActions[2], by: p.vault.custodyBy[2], date: '15 ABR 11:40', ok: true },
                    ].map((s) => (
          ...
          {/* ── GRAFO DE ENTIDADES ── */}
          {demoTab === 'graph' && (
            <DemoShell title={p.graph.title} badge={p.graph.badge} badgeColor="text-red-400" pulse={pulse}>
              ...
              <div className="relative h-80 overflow-hidden bg-[radial-gradient(ellipse_at_center,rgba(0,212,255,0.03)_0%,transparent_70%)] sm:h-96">
                <svg className="absolute inset-0 h-full w-full" viewBox="0 0 800 400" preserveAspectRatio="xMidYMid meet">
                  ...
                  {/* Edge labels */}
                  <text x="295" y="142" fill="rgba(0,212,255,0.5)" fontSize="8" textAnchor="middle" fontFamily="monospace">{p.graph.edges[0]}</text>
                  <text x="508" y="142" fill="rgba(139,92,246,0.6)" fontSize="8" textAnchor="middle" fontFamily="monospace">{p.graph.edges[1]}</text>
                  <text x="520" y="228" fill="rgba(34,197,94,0.5)" fontSize="8" textAnchor="middle" fontFamily="monospace">{p.graph.edges[2]}</text>
                  <text x="285" y="260" fill="rgba(251,191,36,0.6)" fontSize="8" textAnchor="middle" fontFamily="monospace">{p.graph.edges[3]}</text>
                  <text x="400" y="278" fill="rgba(239,68,68,0.7)" fontSize="8" textAnchor="middle" fontFamily="monospace" fontWeight="bold">{p.graph.edges[4]}</text>
                  ...
                  <text x="400" y="197" fill="#00d4ff" fontSize="9" textAnchor="middle" fontFamily="monospace" fontWeight="bold">{p.graph.operationWord}</text>
                  ...
                  <text x="400" y="375" fill="rgba(239,68,68,0.5)" fontSize="7" textAnchor="middle" fontFamily="monospace">{p.graph.suspectLabel}</text>
                  ...
                  <text x="600" y="118" fill="rgba(139,92,246,0.5)" fontSize="7" textAnchor="middle" fontFamily="monospace">{p.graph.nodeOrg}</text>

                  {/* Node: Financial */}
                  <rect x="140" y="75" width="120" height="50" rx="4" fill="rgba(0,212,255,0.08)" stroke="rgba(0,212,255,0.4)" strokeWidth="1.5"/>
                  <text x="200" y="97" fill="rgba(0,212,255,0.9)" fontSize="8" textAnchor="middle" fontFamily="monospace" fontWeight="bold">{p.graph.nodeTransfers}</text>
                  <text x="200" y="108" fill="rgba(0,212,255,0.9)" fontSize="8" textAnchor="middle" fontFamily="monospace" fontWeight="bold">$4.2M USD</text>
                  <text x="200" y="118" fill="rgba(0,212,255,0.4)" fontSize="7" textAnchor="middle" fontFamily="monospace">{p.graph.nodeFinancial}</text>
                  ...
                  <text x="620" y="282" fill="rgba(34,197,94,0.4)" fontSize="7" textAnchor="middle" fontFamily="monospace">{p.graph.nodeLocation}</text>
                  ...
                  <text x="180" y="330" fill="rgba(251,191,36,0.4)" fontSize="7" textAnchor="middle" fontFamily="monospace">{p.graph.nodeCompany}</text>

                  {/* Risk indicator */}
                  <rect x="30" y="20" width="90" height="50" rx="4" fill="rgba(239,68,68,0.1)" stroke="rgba(239,68,68,0.3)" strokeWidth="1"/>
                  <text x="75" y="37" fill="rgba(239,68,68,0.7)" fontSize="7" textAnchor="middle" fontFamily="monospace">{p.graph.riskLabel}</text>
                  <text x="75" y="54" fill="#ef4444" fontSize="20" textAnchor="middle" fontFamily="monospace" fontWeight="bold">89</text>
                  <text x="75" y="64" fill="rgba(239,68,68,0.5)" fontSize="7" textAnchor="middle" fontFamily="monospace">{p.graph.riskLevel}</text>

                  {/* Stats */}
                  <text x="680" y="370" fill="rgba(0,212,255,0.3)" fontSize="8" textAnchor="middle" fontFamily="monospace">{p.graph.statsLine}</text>
                </svg>

                {/* Overlay stats */}
                <div className="absolute bottom-3 left-3 flex gap-2">
                  {[
                    { label: p.graph.overlay[0], value: '62', color: 'text-zord-accent' },
                    { label: p.graph.overlay[1], value: '124', color: 'text-purple-400' },
                    { label: p.graph.overlay[2], value: '153', color: 'text-yellow-400' },
                  ].map((s) => (
          ...
          {/* TIMELINE */}
          {demoTab === 'timeline' && (
            <DemoShell title={p.timeline.title} badge={p.timeline.badge} badgeColor="text-green-400" pulse={pulse}>

              <div className="p-4">
                {/* Timeline axis */}
                <div className="mb-4 flex items-center gap-0 overflow-x-auto pb-2">
                  {p.timeline.axis.map((d) => (
                ...
                {[
                  {
                    label: p.timeline.tracks[0],
                    color: 'bg-blue-500/20 border-blue-500/50 text-blue-400',
                    dot: 'bg-blue-400',
                    events: [
                      { pos: 5, label: p.timeline.commEvents[0] },
                      { pos: 22, label: p.timeline.commEvents[1] },
                      { pos: 45, label: p.timeline.commEvents[2] },
                      { pos: 62, label: p.timeline.commEvents[3] },
                      { pos: 80, label: p.timeline.commEvents[4] },
                    ],
                  },
                  {
                    label: p.timeline.tracks[1],
                    color: 'bg-yellow-500/20 border-yellow-500/50 text-yellow-400',
                    dot: 'bg-yellow-400',
                    events: [
                      { pos: 15, label: p.timeline.finEvents[0] },
                      { pos: 38, label: p.timeline.finEvents[1] },
                      { pos: 55, label: p.timeline.finEvents[2] },
                      { pos: 72, label: p.timeline.finEvents[3] },
                    ],
                  },
                  {
                    label: p.timeline.tracks[2],
                    color: 'bg-green-500/20 border-green-500/50 text-green-400',
                    dot: 'bg-green-400',
                    events: [
                      { pos: 12, label: p.timeline.moveEvents[0] },
                      { pos: 42, label: p.timeline.moveEvents[1] },
                      { pos: 65, label: p.timeline.moveEvents[2] },
                    ],
                  },
                  {
                    label: p.timeline.tracks[3],
                    color: 'bg-purple-500/20 border-purple-500/50 text-purple-400',
                    dot: 'bg-purple-400',
                    events: [
                      { pos: 8, label: p.timeline.docEvents[0] },
                      { pos: 30, label: p.timeline.docEvents[1] },
                      { pos: 50, label: p.timeline.docEvents[2] },
                      { pos: 70, label: p.timeline.docEvents[3] },
                      { pos: 88, label: p.timeline.docEvents[4] },
                    ],
                  },
                ].map((track) => (
                ...
                <div className="mt-4 rounded border border-zord-accent/30 bg-zord-accent/5 p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-medium text-zord-accent">{p.timeline.correlationLabel}</span>
                  </div>
                  <p className="mt-1 text-[10px] leading-relaxed text-zord-text-muted">
                    {p.timeline.correlationText}<span className="font-bold text-zord-accent">94%</span>
                  </p>
                </div>
              </div>
            </DemoShell>
          ...
          {/* ── MOTOR DE HIPÓTESES ── */}
          {demoTab === 'hypothesis' && (
            <DemoShell title={p.hypothesis.title} badge={p.hypothesis.badge} badgeColor="text-purple-400" pulse={pulse}>
              <div className="grid grid-cols-1 gap-0 sm:grid-cols-3">
                {/* Hypothesis list */}
                <div className="col-span-2 space-y-4 border-r border-zord-border p-5">
                  {[
                    { id: 'H1', title: p.hypothesis.items[0], score: 94, ev: 12, ce: 2, level: p.severity.critical, color: 'text-red-400', bar: 'bg-red-500', border: 'border-red-700/50 bg-red-950/10' },
                    { id: 'H2', title: p.hypothesis.items[1], score: 78, ev: 8, ce: 1, level: p.severity.high, color: 'text-orange-400', bar: 'bg-orange-500', border: 'border-orange-700/50 bg-orange-950/10' },
                    { id: 'H3', title: p.hypothesis.items[2], score: 61, ev: 5, ce: 3, level: p.severity.medium, color: 'text-yellow-400', bar: 'bg-yellow-500', border: 'border-yellow-700/50 bg-yellow-950/10' },
                    { id: 'H4', title: p.hypothesis.items[3], score: 45, ev: 4, ce: 4, level: p.severity.low, color: 'text-zord-text-muted', bar: 'bg-zord-border', border: 'border-zord-border bg-zord-surface/20' },
                  ].map((h) => (
                    <div key={h.id} className={`rounded border p-4 ${h.border}`}>
                      ...
                      <div className="flex gap-4 text-[9px]">
                        <span className="text-green-400">✓ {h.ev} {p.hypothesis.corroborate}</span>
                        <span className="text-red-400">✗ {h.ce} {p.hypothesis.contradict}</span>
                        <span className="text-zord-text-muted">· {h.ev + h.ce} {p.hypothesis.total}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="p-5">
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-widest text-purple-400">{p.hypothesis.aiLabel}</span>
                  </div>
                  <div className="rounded border border-purple-700/40 bg-purple-950/10 p-3 text-[10px] leading-relaxed text-zord-text-muted mb-4">
                    <span className="font-semibold text-purple-400">{p.hypothesis.aiTextBold}</span>{p.hypothesis.aiText}
                  </div>
                  <div className="mb-3 text-[9px] font-bold uppercase tracking-widest text-zord-text-muted">{p.hypothesis.gapsLabel}</div>
                  {[
                    { t: p.hypothesis.gaps[0], s: p.severity.high, sk: 'high' },
                    { t: p.hypothesis.gaps[1], s: p.severity.medium, sk: 'medium' },
                    { t: p.hypothesis.gaps[2], s: p.severity.low, sk: 'low' },
                  ].map((g) => (
                    <div key={g.t} className="mb-2 flex items-start gap-2 rounded border border-zord-border/50 bg-zord-surface/20 p-2">
                      <span className={`mt-0.5 text-[8px] font-bold ${g.sk === 'high' ? 'text-red-400' : g.sk === 'medium' ? 'text-yellow-400' : 'text-zord-text-muted'}`}>{g.s}</span>
                      <span className="text-[9px] text-zord-text-muted">{g.t}</span>
                    </div>
                  ))}
                  <div className="mt-4 rounded border border-zord-accent/30 bg-zord-accent/5 p-3">
                    <div className="text-[9px] font-bold text-zord-accent mb-1">{p.hypothesis.recommendationLabel}</div>
                    <p className="text-[9px] leading-snug text-zord-text-muted">{p.hypothesis.recommendationText}</p>
                  </div>
                </div>
              </div>
            </DemoShell>
          ...
          {/* ── CENTRO DE OPERAÇÕES ── */}
          {demoTab === 'ops' && (
            <DemoShell title={p.ops.title} badge={p.ops.badge} badgeColor="text-red-400" pulse={pulse}>
              {/* Top bar */}
              <div className="grid grid-cols-4 border-b border-zord-border">
                {[
                  { v: '3', l: p.ops.stats[0], c: 'text-zord-accent' },
                  { v: '11', l: p.ops.stats[1], c: 'text-green-400' },
                  { v: '8', l: p.ops.stats[2], c: 'text-red-400' },
                  { v: '94%', l: p.ops.stats[3], c: 'text-purple-400' },
                ].map((s) => (
              ...
              <div className="grid grid-cols-1 gap-0 sm:grid-cols-3">
                {/* Operations */}
                <div className="col-span-2 border-r border-zord-border p-4">
                  <div className="mb-3 text-[9px] font-bold uppercase tracking-widest text-zord-text-muted">{p.ops.operationsLabel}</div>
                  <div className="space-y-3">
                    {[
                      {
                        id: 'OP-2024-001', name: p.ops.names[0], cls: p.classification.secret, status: p.status.critical,
                      ...
                      {
                        id: 'OP-2024-007', name: p.ops.names[1], cls: p.classification.confidential, status: p.status.high,
                      ...
                      {
                        id: 'OP-2024-011', name: p.ops.names[2], cls: p.classification.confidential, status: p.status.medium,
                      ...
                      <div key={op.id} className={`rounded border p-3 ${op.color}`}>
                        <div className="mb-2 flex items-start justify-between gap-2">
                          ...
                          <div className="shrink-0 text-right">
                            ...
                            <div className="text-[8px] text-zord-text-muted">{p.ops.trustScore}</div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1 text-[8px] text-zord-text-muted">
                            <span>👤</span><span>{op.analistas} {p.ops.analysts}</span>
                          </div>
                        </div>
                ...
                <div className="p-4">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-widest text-red-400">{p.ops.feedLabel}</span>
                  </div>
                  <div className="space-y-2">
                    {[
                      { t: '14:38', op: 'NIGHTHAWK', msg: p.ops.feed[0], sev: p.severity.critical, sc: 'text-red-400 border-red-700/50 bg-red-950/20' },
                      { t: '14:35', op: 'NIGHTHAWK', msg: p.ops.feed[1], sev: p.severity.critical, sc: 'text-red-400 border-red-700/50 bg-red-950/20' },
                      { t: '14:22', op: 'ALFA', msg: p.ops.feed[2], sev: p.severity.high, sc: 'text-orange-400 border-orange-700/50 bg-orange-950/20' },
                      { t: '13:58', op: 'NIGHTHAWK', msg: p.ops.feed[3], sev: p.severity.high, sc: 'text-orange-400 border-orange-700/50 bg-orange-950/20' },
                      { t: '13:44', op: 'TEMPESTADE', msg: p.ops.feed[4], sev: p.severity.medium, sc: 'text-yellow-400 border-yellow-700/50 bg-yellow-950/20' },
                      { t: '13:30', op: 'ALFA', msg: p.ops.feed[5], sev: p.severity.info, sc: 'text-zord-text-muted border-zord-border bg-zord-surface/20' },
                    ].map((ev, i) => (
